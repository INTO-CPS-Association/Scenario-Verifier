  ...
  def createFMUs(FMUStrings: immutable.Iterable[String]): Iterable[FMUState] = {
    modelEncoding.fmuEncoding.map(i => {
      ...
      val inputPorts = modelEncoding.fmuInputEncodingInverse(i._1).map(o => {
        createPortVariable(inputVariables.filter(_.matches(f".*\\[${o._1}\\].*")).mkString(","), i._1, o._2, modelEncoding.fmuModels(i._1).inputs(o._2).reactivity == reactive)
      }).toList
      ...
      val outputPorts = modelEncoding.fmuOutputEncodingInverse(i._1).map(o => {
        createPortVariable(outputVariables.filter(_.matches(f".*\\[${o._1}\\].*")).mkString(","), i._1, o._2, false)
      }).toList
...
object TraceAnalyzer {
  def AnalyseScenario(scenarioName: String, trace: Iterator[String], modelEncoding: ModelEncoding): Unit = {
    ...
    //This filtering could be avoided by changing the Uppaal-model
