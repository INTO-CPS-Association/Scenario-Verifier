...
import core.{ConnectionModel, CosimStepInstruction, InitializationInstruction, OutputPortModel, PortRef, ScenarioModel}
...
case class DoStepNode(override val fmuName: String) extends StepInstructionNode {
  override def formatStepInstruction: CosimStepInstruction = core.Step(fmuName, core.DefaultStepSize())
}

case class GetNode(override val fmuName: String, override val port: PortRef) extends PortNode {
  override def formatInitInstruction: InitializationInstruction = core.InitGet(port)

  override def formatStepInstruction: CosimStepInstruction = core.Get(port)
}
...
case class SetNode(override val fmuName: String, override val port: PortRef) extends PortNode {
  override def formatInitInstruction: InitializationInstruction = core.InitSet(port)

  override def formatStepInstruction: CosimStepInstruction = core.Set(port)
}

case class SetTentativeNode(override val fmuName: String, port: PortRef) extends PortNode {
  override def formatStepInstruction: CosimStepInstruction = core.SetTentative(port)

  override def formatInitInstruction: InitializationInstruction = core.InitSet(port)
}

case class RestoreNode(override val fmuName: String) extends StepInstructionNode {
  override def formatStepInstruction: CosimStepInstruction = core.RestoreState(fmuName)
}

final case class SaveNode(override val fmuName: String) extends StepInstructionNode {
  override def formatStepInstruction: CosimStepInstruction = core.SaveState(fmuName)
}

final case class EmptyNode() extends StepInstructionNode {
  override val fmuName: String = "EmptyNode"

  override def formatStepInstruction: CosimStepInstruction = core.NoOP
}
...
class GraphBuilder(scenario: ScenarioModel, val removeTransitive: Boolean = false) extends Logging {
  ...
  private val SetNodesReactive: Map[String, Set[SetTentativeNode]] =
    scenario.fmus.map(fmu => (fmu._1, fmu._2.inputs.filter(i => i._2.reactivity == reactive).map(i => SetTentativeNode(fmu._1, PortRef(fmu._1, i._1))).toSet))
  private val SetNodesDelayed: Map[String, Set[SetNode]] =
    scenario.fmus.map(fmu => (fmu._1, fmu._2.inputs.filter(i => i._2.reactivity != reactive).map(i => SetNode(fmu._1, PortRef(fmu._1, i._1))).toSet))
  ...
  private def feedthroughByType(dependencyTypes: OutputPortModel => List[String]): Set[Edge[PortNode]] =
    ...
    }).flatMap(f => f._2.map(i => Edge[PortNode](i, f._1))).toSet
  ...
  private def edgesLinkedToStep(
                                 ...
                                 setsDelayed: Map[String, Set[SetNode]]
                               ): Set[Edge[CosimStepInstruction]] = {
  ...
  lazy val initialEdges: Set[Edge[InitializationInstructionNode]] =
    (connectionEdges ++ feedthroughByType(_.dependenciesInit))
      .map(i => Edge[InitializationInstructionNode](i.srcNode.asInstanceOf[InitializationInstructionNode], i.trgNode.asInstanceOf[InitializationInstructionNode]))
