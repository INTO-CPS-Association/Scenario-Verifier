...
case class LoopConfigStep(untilConverged: List[String] = Nil,
...
case class LoopConfigInit(untilConverged: List[String],
                          override val iterate: List[NestedInitStatement],
...
case class NestedStepStatement(get: String = "",
  ...
  require(get.nonEmpty || getTentative.nonEmpty || setTentative.nonEmpty || set.nonEmpty || step.nonEmpty ||
    saveState.nonEmpty || restoreState.nonEmpty || loop.iterate.nonEmpty, "At least one of get, set, step, saveState, or restoreState must be defined")
...
case class RootStepStatement(get: String = "",
                             ...
                             by: Int = -1,
                             bySameAs: String = "") extends StepStatement {
  require(get.nonEmpty || set.nonEmpty || step.nonEmpty || saveState.nonEmpty || restoreState.nonEmpty || loop.iterate.nonEmpty, "At least one of get, set, step, saveState, or restoreState must be defined")
}
...
case class RootInitStatement(get: String = "",
                             set: String = "",
                             loop: LoopConfigInit = NoLoopInit) extends InitializationStatement {
