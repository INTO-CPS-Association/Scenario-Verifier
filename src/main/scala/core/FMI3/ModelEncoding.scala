...
import core.{AlgebraicLoop, AlgebraicLoopInit, ConnectionModel, CosimStepInstruction, FmuModel, InitializationInstruction, InputPortModel, ModelQuery, OutputPortModel, PortRef, StepLoop}
import org.apache.logging.log4j.scala.Logging


/*
class ModelEncoding(model: MasterModel3) extends Logging {
  ...
  def nInternal(f: Fmu3Model): Int = f.outputs.map(o => o._2.dependencies.length).sum[Int]
  ...
  val connections: List[ConnectionModel] = model.scenario.connections
  ...
  def fmuInputTypes(f: String): String = {
    ...
    (0 until nConfigs).map(id => {
      val inputPortConfig: Map[PortRef, InputPortModel] = if (isAdaptive) model.scenario.config.configurations(configuration(id)._1).inputs.filter(_._1.fmu == f) else Map.empty
  ...
  val allAlgebraicLoopsInStep: List[AlgebraicLoop] =
    ModelQuery.recFilter(keyValue._2, i => i.isInstanceOf[AlgebraicLoop]).map(_.asInstanceOf[AlgebraicLoop]))
  ...
  val loopOpsEncoding: Map[Int, AlgebraicLoop] = allAlgebraicLoopsInStep.map(keyValue => (keyValue._1, keyValue._2.zipWithIndex.map(_.swap).toMap))
  val loopOpsEncodingInverse: Map[AlgebraicLoop, Int] = loopOpsEncoding.map(keyValue => (keyValue._1, keyValue._2.map(_.swap)))
  ...
  def retryOperationsPerAlgebraicLoopInStep: String = getAlgebraicLoopOp(getARetryOperations, encodeList, maxNRetryOperationsForAlgebraicLoopsInStep)
  ...
  def stepOperations: String = model.cosimStep.map(_.toUppaal).mkString(",")
