...
object ScenarioPlotter extends Logging {
  ...
  def createInputNode(fmu: String, portName: String, isReactive: Boolean, action: Option[UPPAAL_Action], state: ModelState): MutableNode = {
    ...
    mutNode(nodeName)
      .add(
        ...
        Shape.BOX,
        Label.lines(nodeName, state.portTime(fmu, portName, isInput = true), if (isReactive) "R" else "D"))
  }
  ...
  def createOutputNode(fmu: String, portName: String, action: Option[UPPAAL_Action], state: ModelState): MutableNode = {
    ...
    mutNode(nodeName)
      .add(
        if (isCurrent(action, fmu, portName)) Color.RED
        else if (state.isDefinedOutputState(fmu, portName)) Color.BLACK
        else Color.WHITE,
        Shape.BOX,
        Label.lines(nodeName, state.portTime(fmu, portName, isInput = false)))
  }

  def createGraphOfState(
      state: ModelState,
      currentAction: Option[UPPAAL_Action],
      possibleActions: List[UPPAAL_Action],
      modelEncoding: ModelEncoding,
      name: String): MutableGraph = {
    ...
    modelEncoding.fmuModels.foreach(fmu => {
      ...
      connectionsFromFMU.foreach(connection =>
        g.add(
          mutNode(connection.srcPort.fmu + "." + connection.srcPort.port)
  ...
  def createLegend(): MutableNode = {
    mutNode("Legend")
      .add(Shape.SQUARE, Label.htmlLines("<b>Legend:</b>", "Black: defined", "White: undefined", "Red: current"))
  }
  ...
  def makeAnimation(movie: File, states: Seq[ModelState], modelEncoding: ModelEncoding, scenarioName: String): String = {
    ...
    states.foreach(state => {
      val filteredActions = state.possibleActions
        .filterNot(act =>
          if (state.checksDisabled) false
          else performedActions.exists(per => per.actionNumber == act.actionNumber && per.FMU == act.FMU && act.Port == per.Port))
      enrichedStates += ModelState(
        state.checksDisabled,
        state.loopActive,
        state.timeStamp,
        state.FMUs,
        state.action,
