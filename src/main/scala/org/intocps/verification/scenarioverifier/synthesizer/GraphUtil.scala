...
object GraphUtil {
  ...
  @annotation.tailrec
  private final def bfs[A](
      graph: Map[A, LazyList[A]],
      toVisit: LazyList[A],
      visited: LazyList[A],
      accumulator: LazyList[A]): LazyList[A] = {
    if (toVisit.isEmpty) {
      accumulator
    } else {
  ...
  def removeTransitiveEdges[A](edges: Set[Edge[A]], nonTrivialSCC: Set[A]): Set[Edge[A]] = {
    ...
    val nodes =
      edges.map(o => (o.srcNode, o.trgNode)).groupMap(_._2)(_._1).filter(i => i._2.size > 1).map(i => (i._1, i._2.diff(nonTrivialSCC)))
    nodes.foreach(n => {
      n._2.foreach(v =>
        n._2
          .filter(_ != v)
