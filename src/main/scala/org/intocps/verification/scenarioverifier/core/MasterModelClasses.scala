...
final case class FmuModel(inputs: Map[String, InputPortModel], outputs: Map[String, OutputPortModel], canRejectStep: Boolean, path: String)
    extends ConfElement {
  require(
...
final case class MasterModel(
  ...
  def toSMTLib(algorithmTypes: List[AlgorithmType], produceModel: Boolean = false): String = {
    ...
    val coSimStepConstraints: String = if (algorithmTypes.contains(AlgorithmType.step)) {
      ...
      val stepInstructions = first_cosimStep
        .filter(instruction =>
          instruction.isInstanceOf[Get] || instruction.isInstanceOf[org.intocps.verification.scenarioverifier.core.Set] || instruction
            .isInstanceOf[Step])
        .map(_.toSMTLib)
...
case class ConfigurationModel(inputs: Map[PortRef, InputPortModel], cosimStep: String, connections: List[ConnectionModel])
